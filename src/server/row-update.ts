...
async function resolveEditTarget(edit: RowEdit): Promise<EditTarget> {
  ...
  for (const change of edit.changes) {
    ...
    // An edit is the text Postgres will read, and text cast to `bytea` is stored
    // as its own characters — so the hex (or the decoded document, where the
    // column is compressed) that the page showed would be written over the bytes
    // it was a rendering of.
    if (col.dataType.toLowerCase() === 'bytea') {
      return {
        ok: false,
        error: `${change.column} holds bytes; this editor writes text, which Postgres would store as the characters themselves.`,
      }
    }
  }
